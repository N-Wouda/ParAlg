...
bounds const blockBounds(bounds const *bounds, size_t numProcs, size_t pid)
{
    ...
    size_t edgeCase = 2* ((bounds->lowerBound % 6) == 1 || (bounds->lowerBound % 6)== 0)
    ...
    size_t upperBound = pid == numProcs - 1
                        ? bounds->upperBound
                        : bounds->lowerBound + (pid + 1) * blockSize;
    edgeCase = ((upperBound % 6) == 1 || (upperBound % 6) == 0|| (upperBound % 6) == 5);
    size_t const blockUpperBound = upperBound - 3 * edgeCase;
